...
class Meta {
 public:
  ...
  // virtual types::ErrCode update_inode_size(types::InodeID ino, uint64_t size,
  // bool sync) = 0; virtual types::ErrCode refresh_inode(types::InodeID ino) =
  // 0; virtual types::ErrCode remove_inode(types::InodeID ino) = 0;

  // virtual types::ErrCode rename(types::InodeID oldPino, std::string oldName,
  // types::InodeID newPino, std::string newName) = 0; virtual types::ErrCode
  // load_sub_dentires(types::InodeID ino, types::Dirent dst[]) = 0; virtual
  // void destory() = 0;
};
