...
public:
  MetaImpl(const MetaOpt &opt, std::shared_ptr<LocalMeta> lm,
           std::shared_ptr<stor::Stor> stor) {
    this->opt = opt;
    this->stor = stor;
    this->local_meta = lm;
  ...
  Dentry *find_dentry(InodeID pino, const std::string &name,
                      bool onlyLocal) override;
  Dentry *create_dentry(InodeID pino, const std::string &name,
                        mode_t mode) override;
  void remove_dentry(InodeID pino, const std::string &name) override;
  Dentry *get_dentry(InodeID ino) override;
  void load_sub_dentries(InodeID ino,
                         std::vector<types::Dirent> &dirents) override;

  void get_remote_obj(Dentry &parent, const std::string &name,
  ...
  void build_ftree_from_listobjects(std::string &rep_prefix,
                                    stor::ListObjectsResp &ret,
